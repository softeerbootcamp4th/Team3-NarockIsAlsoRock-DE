...
def post_crawling(driver, title, url, datetime_start, datetime_end): # 게시물 하나에 대한 크롤링
    ...
    except: # 계정의 등급 문제 등으로 게시물을 읽을 수 없는 경우 / 설정한 날짜 범위를 벗어나는 경우
        return None, None
...
def main():   
    timestamp_datetime_start = datetime.strptime('2024.08.08. 00:00', "%Y.%m.%d. %H:%M")
    timestamp_datetime_end = datetime.strptime('2024.08.08. 23:59', "%Y.%m.%d. %H:%M")
